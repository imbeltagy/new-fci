...
import type { StaffSubjectEntry } from "@repo/common/types/subject";
...
export function CreateAssessmentView() {
  ...
  const subjects: StaffSubjectEntry["subject"][] =
    ((mySubjectsData?.data as any)?.subjects as StaffSubjectEntry[] | undefined)?.map((e) => e.subject) ?? [];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 py-4 max-w-md">
      <h1 className="text-lg font-bold">Create Assessment</h1>
      ...
      <div>
        <Label>Title</Label>
        <Input value={title} onChange={(e) => setTitle(e.target.value)} required placeholder="Assessment title" />
      </div>
      ...
      {type === "assignment" && (
        <div>
          ...
          <Input type="number" min={1} value={totalMark} onChange={(e) => setTotalMark(e.target.value)} required />
        </div>
      )}
